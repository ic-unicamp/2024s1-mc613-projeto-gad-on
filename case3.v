	  3: begin // cor = 17
				...
				if(rot_id == 0) begin
					...
					if(faz_alguma_coisa) begin //movimento para os lados (mef com keys)
						if(~KEY[1] && x_shape > 5 && map[y_shape-1][(x_shape-10)+:5] == 0 && map[y_shape][(x_shape-5)+:5] == 0) begin
							x_shape <= x_shape - 5;
						end
						else if(~KEY[0] && x_shape < 40 && map[y_shape-1][(x_shape+5)+:5] == 0 && map[y_shape][(x_shape+10)+:5] == 0) begin
							x_shape <= x_shape + 5;
						end
					...
					if(map[y_shape][(x_shape-5)+:5] != 0 || map[y_shape+1][x_shape+:5] != 0 || map[y_shape+1][(x_shape+5)+:5] != 0 || y_shape == 19) begin 
						map[y_shape-1][(x_shape-5)+:5] <= 7;
						map[y_shape-1][x_shape+:5] <= 7;
						map[y_shape][x_shape+:5] <= 7;
						map[y_shape][(x_shape+5)+:5] <= 7;
					...
					if(faz_alguma_coisa) begin //movimento para os lados (mef com keys)
						if(~KEY[1] && x_shape > 5 && map[y_shape][(x_shape-10)+:5] == 0 && map[y_shape+1][(x_shape-5)+:5] == 0) begin				
							x_shape <= x_shape - 5;
						end
						else if(~KEY[0] && x_shape < 45 && map[y_shape][(x_shape+5)+:5] == 0 && map[y_shape-1][(x_shape+5)+:5] == 0) begin
							x_shape <= x_shape + 5;
						end
						else if(~KEY[2] && x_shape < 45 && map[y_shape-1][(x_shape-5)+:5] == 0 && map[y_shape][(x_shape+5)+:5] == 0) begin
							rot_id <= 0;
						end
					end
